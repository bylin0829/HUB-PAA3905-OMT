...
#include "hub5168p.h"
#include "PAA3905_lib.h"                // 使用PAA3905程式庫
...
void loop() {
    ...
    if (millis() - timer > 100) { // print data every 0.1 second
...
#ifndef BRUST_READ
        if (op = paa.readMotion(&dx, &dy)) {          //讀取PAA3905資料
        ...
        DBG("OP=%d Sql=%d Sht=%d Dx=%d Dy=%d Temp=%f angleX=%f angleY=%f angleZ=%f accX=%f accY=%f accZ=%f", op, sql, sht, dx, dy, mpu.getTemp(), mpu.getAngleX(), mpu.getAngleY(), mpu.getAngleZ(), mpu.getAccX(), mpu.getAccY(), mpu.getAccZ());
        timer = millis();
    }
}
